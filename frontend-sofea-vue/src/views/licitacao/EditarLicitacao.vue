<template>
<!-- TODO - Implementar mensagens/notificacoes para o usuario aqui nessa tela -->
<!-- TODO - Tratar possiveis excecoes disparadas pela API -->
    <div>
        <Navbar />
        <br />
        <div class="container">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <b>EDITAR LICITAÇÃO</b>
            </div>
            <form>
                <div class="mb-3">
                    <label for="descricao">Descrição</label>
                    <input type="email" class="form-control" placeholder="descrição" v-model="licitacaoBuscada.descricao">
                </div>
                <div class="row">
                    <div class="col-md-5 mb-3">
                        <label for="classificacao">Tipo de Classificação</label>
                        <select class="custom-select d-block w-100" id="country" v-model="licitacaoBuscada.tipoClassificacao">
                            <option :value="tipo" v-for="(tipo, index) in tiposClassificacao" v-bind:key="index">
                                {{ tipo }}
                            </option>
                        </select>
                    </div>
                </div>
                <button type="button" class="btn btn-primary" v-on:click="save">Salvar</button>
                <router-link to="/licitacoes/" class="btn btn-danger">Cancelar</router-link>
            </form>
        </div>
    </div>
</template>

<script>
import Navbar from '@/components/Navbar'
import LicitacaoService from '@/services/licitacaoservice'

export default {
  name: 'app',
  components: {
    Navbar
  },
  data() {
      return {
          tiposClassificacao: [],
          licitacaoBuscada: {}
      }
  },
  mounted() {
      LicitacaoService.buscarPeloId(this.id).then(response => {
      this.licitacaoBuscada = response.data
    }),
    LicitacaoService.listarTiposClassificacao().then(response => {
        this.tiposClassificacao = response.data
    })
  },
  methods: {
      save(){
          LicitacaoService.editar(this.licitacaoBuscada).then(response => {
              alert('Alterado com sucesso! :)');
              response
          })
      }
  },
  props: ['id']
}
</script>

<style></style>
